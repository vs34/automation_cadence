...
def initialize_matrix():
    ...
    # return mat
    file = open('Pattern.il','r')
    csvreader = csv.reader(file)
    for a in csvreader:
	if (a == []):
	    break
	row = [element.strip() for element in a]
	mat.append(row)
    return mat[::-1]
...
def calculate_matching_values(mat, div, operation_func):
    ...
    sum_dict = {}
    center = operation_func(len(mat)/2,len(mat[0])/2)
    for x in range(len(mat)):
        for y in range(len(mat[0])):
	    if mat[x][y] == '*':
		continue
            if mat[x][y] not in sum_dict:
                sum_dict[mat[x][y]] = operation_func(x, y) - center
            else:
                sum_dict[mat[x][y]] += operation_func(x, y) - center
    return sum_dict
...
def plot(sum_dict, div, description):
    ...
    gnuplot_script = """
        set title "Matching for {0}" font ",18"
        set xlabel "Device" font ",15"
        set ylabel "Average" font ",15"
        set grid
...
def list_plot(dict_div_sum, div):
	...
	set title font ",200"
        ...
        unset multiplot
        pause -1
    """.format(int(len(dict_div_sum) ** 0.5), int(len(dict_div_sum) ** 0.5) if int(len(dict_div_sum) ** 0.5)**2 == len(dict_div_sum) else int(len(dict_div_sum) ** 0.5) + 1, allplots)
