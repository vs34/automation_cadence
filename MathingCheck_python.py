...
def initialize_matrix():
    ...
    for x in range(a):
        mat.append(list(raw_input(f"Enter row {x+1}: ")))
    return mat
...
def print_results(div, sum_dict, description):
    ...
    for key in div.keys():
        print "Matching for device for {} {} is {}".format(description, key, sum_dict[key] / float(div[key]))
...
def plot(sum_dict, div, description):
    ...
    with open("plot_data.txt", "w") as f:
        for i, key in enumerate(sum_dict.keys(), start=1):
            f.write("{} \"{}\" {}\n".format(i, key, sum_dict[key] / float(div[key])))  # Assign numeric values for the x-axis, but still display string labels
...
def clubed_plot(dict_div_sum, div):
    ...
    for relation in dict_div_sum:
        ...
        for divise in dict_div_sum[relation]:
                MAX_VALUE = max(MAX_VALUE, dict_div_sum[relation][divise] / float(div[divise]))
                MIN_VALUE = min(MIN_VALUE, dict_div_sum[relation][divise] / float(div[divise]))
        for divise in dict_div_sum[relation]:
            if MAX_VALUE == MIN_VALUE:
                dict_div_sum[relation][divise] = 0
                continue
            dict_div_sum[relation][divise] = (dict_div_sum[relation][divise] / float(div[divise]) - MIN_VALUE) / (MAX_VALUE - MIN_VALUE) * (MAX_GRAPH - MIN_GRAPH) + MIN_GRAPH
    allplots = ''
    line_style = 1
    for relation in dict_div_sum:
        with open(relation+".txt","w") as f:
            for i, key in enumerate(dict_div_sum[relation].keys(), start=1):
                f.write("{} \"{}\" {}\n".format(i, key, dict_div_sum[relation][key]))  # normalize this value to be max equal to MAX_GRAPH mand min to MIN_GRAPH
        allplots += '"{}.txt" using 1:3:xtic(2) title "{} values" with line linestyle {},'.format(relation, relation, line_style)
        line_style += 1

    gnuplot_script = """
        set title "Normalized Matchings (MAX = {}, MIN = {})"
        set xlabel "Device"
        set ylabel "Average"
        set grid
        set xtics format "%s"  # Format x-tics as strings
...
def list_plot(dict_div_sum, div):
    ...
    for relation in dict_div_sum:
        with open(relation+".txt","w") as f:
            for i, key in enumerate(dict_div_sum[relation].keys(), start=1):
                f.write("{} \"{}\" {}\n".format(i, key, dict_div_sum[relation][key] / float(div[key])))  # Assign numeric values for the x-axis, but still display string labels
        allplots += """plot "{}.txt" using 1:3:xtic(2) with linespoints title "{} values"
        """.format(relation, relation)

    gnuplot_script = """
        set xlabel "Device"
        set ylabel "Average"
        set multiplot layout {},{} columns
        set xtics format "%s"  # Format x-tics as strings
        {}
        unset multiplot
        pause -1
    """.format(int(len(dict_div_sum)**0.5), int(len(dict_div_sum)**0.5) if (int(len(dict_div_sum))**2 == len(dict_div_sum)) else  int(len(dict_div_sum)**0.5+1), allplots)
